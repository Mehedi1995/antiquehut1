...
if ($paidstatus=="true"){
  ...
  $signing = '';
    ...
    if ($signed === $data['x_signature']) {
        ...
        $sqlcart ="SELECT * FROM PRODUCT_ORDER WHERE EMAIL = '$userid'";
        ...
        while ($row = $cartresult->fetch_assoc())
        {
            $id = $row["PRODUCTID"];
            $qty = $row["PRODUCTQTY"];
             $shopid = $row["SHOPID"];//cart qty

            $sqlinsertintopaid = "INSERT INTO PAID(EMAIL,PRODUCTID,PRODUCTQTY) VALUES ('$userid','$id','$qty','shopid')";
            $conn->query($sqlinsertintopaid);
            
        }
        }
        
        $sqldeletecart = "DELETE FROM PRODUCT_ORDER WHERE EMAIL = '$userid'";
     ...
     <p><center>Press back button to return to Antiquehut</center></p></div></body>';
}
else{
     echo 'Payment Failed!';
}
